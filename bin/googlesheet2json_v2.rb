...
PATIENT_MUNICIPALITIES = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:PATIENT_MUNICIPALITIES])
raise if PATIENT_MUNICIPALITIES.empty?

POSITIVE_BY_DIAGNOSED = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:POSITIVE_BY_DIAGNOSED])
raise if POSITIVE_BY_DIAGNOSED.empty?

POSITIVE_RATE = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:POSITIVE_RATE])
raise if POSITIVE_RATE.empty?

HOSPITALIZED_NUMBERS = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:HOSPITALIZED_NUMBERS])
raise if HOSPITALIZED_NUMBERS.empty?

NEWS = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:NEWS])
raise if NEWS.empty?

ALERT = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:ALERT])
# raise if ALERT.empty?

URLS = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:URLS])
raise if URLS.empty?

SELF_DISCLOSURES = Ractor.make_shareable GoogleSheets.data(GoogleSheetsIwate::SHEET_RANGES[:SELF_DISCLOSURES])
...
r_patient_municipalities = Ractor.new name: 'patient_municipalities' do
  patient_municipalities, now = Ractor.receive
  patient_municipalities_json(patient_municipalities, now)
end
r_patient_municipalities.send [PATIENT_MUNICIPALITIES, NOW]
...
r_main_summary = Ractor.new name: 'main_summary' do
  patients, positive_by_diagnosed, hospitalized_numbers, now = Ractor.receive
  main_summary_json(patients, positive_by_diagnosed, hospitalized_numbers, now)
end
r_main_summary.send [PATIENTS, POSITIVE_BY_DIAGNOSED, HOSPITALIZED_NUMBERS, NOW]

# confirmed_case_area.json
# confirmed_case_age.json
r_confirmed_case_area_age = Ractor.new name: 'confirmed_case_area_age' do
  positive_rate, patients, now = Ractor.receive
  confirmed_case_area_age_json(positive_rate, patients, now)
end
r_confirmed_case_area_age.send [POSITIVE_RATE, PATIENTS, NOW]
